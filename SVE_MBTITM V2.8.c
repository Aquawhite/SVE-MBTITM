//*****************************************************************************
// V2.8 External Feeder Timer Mode fixed. Now not active when machine running
// 		MachineStopping message added
// 		able to detect pulse reverse
// 		Deleted ZeroIndex32, ZeroIndex16.. etc
// 		Add Stop Log
// 		Add Save/load parameter in HMI
// 		Add CounterCutterRunButtonEnable
//		Add password to main menu in HMI
...
unsigned char CounterCutterRunButtonEnable;
unsigned int CounterCutterRunButtonTimer;
...
unsigned char PulseReverseErrorMsgDuration, PulseReverseErrorMsg;
/* Debug Data */
...
// Communication SPI
unsigned int SPIOut[26], SPIIn[26];
...
/* Error Log */
unsigned char StopLog, StopLogSent;
	            ...
	            switch(dataRXIndex)
	            {
	                ...
	                case 102:
	                    if(ComAddress == 1)
	                    {
	                    	...
	                    	CounterCutterRunButtonEnable =  (ComIN[102] >> 7) % 2;
	                    }
	                    break;
	                case 103:
	                    if(ComAddress == 1)
...
void IntGPIOc(void)
{
	...
	if( (ulStatus & PIN_PULSE_A_PIN) == PIN_PULSE_A_PIN)
	{
		...
		EncoderDirection = ROM_QEIDirectionGet(QEI1_BASE);
		if(EncoderBuffer < 0)
...
void Timer0IntHandler(void)
{
	...
	if(EncoderDirection == 255)
		PulseReverseErrorMsgDuration = 30;
	/* Encoder Module ^ */
	...
	if(!ms100counter)
	{
		...
		if(CounterCutterRunButtonTimer)
			CounterCutterRunButtonTimer--;
		...
		if(ExternalFeederEnable)
		{
			if(MachineRunning || (!ExternalFeederManualEnable))		// Operates Full on Machine Running, and Still operates when not running but if the Manual is disabled
			{
				...
				{	// Timer Mode (External Feeder is Driven by Timer)
					...
					if(ExternalFeederOFFTimerCounter && MachineRunning)	// Duration will be refilled only on Machinerunning V2.8
					{
		...
		if(PulseReverseErrorMsgDuration)
		{
			PulseReverseErrorMsgDuration--;
			PulseReverseErrorMsg = TRUE;
		}
		else
			PulseReverseErrorMsg = FALSE;
...
main(void)
{
	...
	*/
    ...
    while(1)
    {
        ...
        if((!ServiceMode) && (ComFlag))                                                                               // Operation Page (Not Service Mode)
        {
            if((!MachineRunning) && (!ServoDrain))                                                                             // when machine is not running
            {
                if( MotorONBtn || !RunButton ) // (!PINBTNRUN)
                {
                    ...
                    if(Error == FALSE)                             // Run Button is pushed
                    {
                        ...
                        StopLogSent = FALSE;
                        CounterCutterRunButtonTimer = 15;	// ignore run button for 1.5 secs for zeroing counter cutter
            ...
            else                                                                                                    // when machine is RUNNING
            {
                if(MotorOFFBtn || Error || !StopButton)         // Motor OFF Btn/Inching pushed or Error Triggered
                {
                    ...
                    {
                    	// Sent msg to HMI to record the stop log
                    	if(!StopButton)
                    		StopLog = 1;
                    	else if(MotorOFFBtn)
                    		StopLog = 2;
							...
							StopLog = 11;
						else if(EyemarkError)
							StopLog = 12;
						else if(ZIError)
							StopLog = 13;
						else if(HopperError)
							StopLog = 14;
						else if(ServoValveErrorMsg)
							StopLog = 15;
        ...
        if(MachineRunning)									// when run button pressed, counter cutter triggers on the next cycle
        	if(CounterCutterRunButtonEnable)
				if(!CounterCutterRunButtonTimer)
					if(!RunButton)
						CounterCutterCounter = 0;
        ...
        if(Error || FatalError || PulseCommutationError || HeaterErrorMsg || FoilJamMsg || PulseErrorMsg || NoAirErrorMsg || PulseReverseErrorMsg)
        {
        ...
        {   //Init Page ComOUT[0] - ComOUT[19]
            ...
            ComOUT[39] = PulseReverseErrorMsg + (StopLogSent << 1) + (IsMBTITMPCB << 6) + (StopCommand << 7);
            // All Pins
            if(IsMBTITMHMI)
            {
			...
			ComOUT[65] = 0;
			ComOUT[66] = 0;
			ComOUT[67] = EncoderDirection;		// debug for PulseReverseErrorMsg, remove when done
			if(IsMBTITMHMI)
